with (obj_player1)
{
    if ((place_meeting((x + hsp), y, other.id) || place_meeting((x + xscale), y, other.id)) && (state == (18 << 0) || state == (126 << 0) || state == (128 << 0)))
    {
        with (other)
        {
    ...
    if (state == (25 << 0) || state == (126 << 0) || state == (128 << 0))
    {
    ...
    if (state == (128 << 0) && rocketup == 1 && place_meeting(x, (y - vsp), other.id))
    {
        with (other)
        {
            initialhsp = 8 * sign(other.image_yscale)
            initialvsp = random_range(-4, -1)
            event_user(0)
        }
    }
