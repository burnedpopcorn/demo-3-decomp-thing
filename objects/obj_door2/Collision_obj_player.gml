if (!(place_meeting(x, y, obj_doorblocked)))
{
    with (other.id)
    {
        if (key_up && (state == 0 || state == 69 || state == 70 || state == 91 || state == 65) && y == (other.y + 50) && (!instance_exists(obj_noisesatellite)) && (!instance_exists(obj_fadeout)) && state != 78 && state != 61 && ((obj_player1.spotlight == 1 && object_index == obj_player1) || (obj_player1.spotlight == 0 && object_index == obj_player2)))
        {
            ...
            obj_player1.xscale = other.hallwaydirection
            obj_player1.hallwaydirection = other.hallwaydirection
            obj_player.state = 78
            obj_player.mach2 = 0
            if (instance_exists(obj_player2) && global.coop == 1)
            {
                obj_player2.xscale = other.hallwaydirection
                obj_player2.hallway = 1
                obj_player2.hallwaydirection = other.hallwaydirection
                if (object_index == obj_player2)
                {
